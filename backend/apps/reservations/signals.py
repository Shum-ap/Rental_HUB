...
def update_property_availability_on_booking(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        property_obj.status = Listing.AvailabilityStatus.RESERVED
        property_obj.save(update_fields=["status"])
        logger.info(f"[SIGNAL] Listing '{property_obj.title}' is now unavailable (booked).")
...
def restore_property_availability_on_booking_delete(sender, instance, **kwargs):
    ...
    property_obj.save(update_fields=["status"])
    logger.info(f"[SIGNAL] Listing '{property_obj.title}' is available again.")
